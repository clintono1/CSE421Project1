...
/* Transitions a blocked thread T to the ready-to-run state.
   ...
   update other data. */
void
thread_unblock (struct thread *thr) 
{
  enum intr_level old_level;
...
//  enum intr_level prevLevel = intr_disable ();
  ...
  list_insert_ordered(&ready_list, &thr->elem,
...
int coefficient = div_fp(mult_mixed(load_avg,2),add_mixed(mult_mixed(load_avg,2),1));

int term= mult_mixed(coefficient, t->recent_cpu);
